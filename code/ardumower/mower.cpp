...
#define pinRemoteSteer 0                 // remote control steering (ShieldBuddy) 
#define pinRemoteSpeed 10                // remote control speed
//#define pinRemoteSwitch 52             // remote control switch
#define pinRemoteSwitch 1                // remote control switch (ShieldBuddy)
...
#define pinOdometryLeft 52               // left odometry sensor (ShieldBuddy)
#define pinOdometryLeft2 DAC1            // left odometry sensor (optional two-wire)
//#define pinOdometryRight CANRX         // right odometry sensor  
#define pinOdometryRight 11              // right odometry sensor (ShiledBuddy)
...
// Bluetooth: Shieldbuddy Serial1 (TX2, RX2)
// GPS: Serial3 (TX3, RX3) 
...
{ 
  ...
  */
  ...
  batFactor                 = 0.1287;    // battery conversion factor  / 10 due to arduremote bug, can be removed after fixing (look in robot.cpp)
  batChgFactor              = 0.1287;    // battery conversion factor  / 10 due to arduremote bug, can be removed after fixing (look in robot.cpp)
...
void Mower::setup()
{
 ...
 // drops
  ...
  //pinMode(pinOdometryLeft2, INPUT_PULLUP);    // unbenutzt!
  pinMode(pinOdometryRight, INPUT);
  //pinMode(pinOdometryRight2, INPUT_PULLUP);   // unbenutzt!
...
//    //rc.initSerial(ESP8266_BAUDRATE);
  }
...
//	  // Wenn odometryUse == 1 dann:
	  if (odometryUse)
	  { 
	    attachInterrupt(pinOdometryLeft, odoISR_left, FALLING);
      attachInterrupt(pinOdometryRight, odoISR_right, FALLING);
	  }
//    //-------------------------------------------------------------------------	
//    // mower motor speed sensor interrupt
//	  //-------------------------------------------------------------------------
...
void Mower::setActuator(char type, int value)
{
  switch (type)
  {
    case ACT_MOTOR_MOW:                                                                                         // Motortreiber einstellung - bei Bedarf ändern z.B setL298N auf setMC33926
    
      setMC33926(pinMotorMowDir, pinMotorMowPWM, value);
      break;
    
    case ACT_MOTOR_LEFT:                                                                                        // Motortreiber einstellung - bei Bedarf ändern z.B setL298N auf setMC33926
      
      setMC33926(pinMotorLeftDir, pinMotorLeftPWM, value);
      break;
    
    case ACT_MOTOR_RIGHT:                                                                                       // Motortreiber einstellung - bei Bedarf ändern z.B setL298N auf setMC33926
